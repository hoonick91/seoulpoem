...
router.post('/:idarticles', async (req, res) => {
  try {
      ...
      let query2 = 'select users.iduseers as users from seoul_poem.users where email = ?';
      ...
      res.status(201).json({status : "success", mag: "bookmark success"});
...
router.get('/search', async (req, res) => {
  try {
      ...
      //userid로 변경할 것!!!
      let query1 = 'SELECT seoul_poem.bookmarks.articles_idarticles as idarticles FROM seoul_poem.seoul_poem.bookmarks where seoul_poem.users.email = ? and seoul_poem.bookmarks.users_idusers = seoul_poem.users.idusers';
      var selected = await connection.query(query1,email);

      let query2 = 'SELECT articles.idarticles, pictures.photo, users.profile, articles.title FROM seoul_poem.articles, seoul_poem.users ,seoul_poem.pictures where idarticles = ?  and articles.users_idusers = users.idusers and articles.pictures_idpictures = pictures.idpictures;';
      let bookmark_list=[];

      let len_ = selected.length;
      for(var i=0; i<len_; i++){
        bookmark_list[i] = await connection.query(query2, selected[i].idarticles);
      }
